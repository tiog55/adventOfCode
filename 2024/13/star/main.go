...
func main() {
	...
	file, err := os.Open("2024/13/star/input.txt")
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
	...
	for i, row := range input {
		...
		if strings.HasPrefix(row, "Button A") {
			...
			secondSplit := strings.Split(firstSplit[1], ", ")
			machine.price = Price{x: toInt(strings.Split(secondSplit[0], "=")[1]) + 10000000000000, y: toInt(strings.Split(secondSplit[1], "=")[1]) + 10000000000000}
		} else if len(row) == 0 {
